...
/**
 * `state.files` mixes single paths with `string[]` lists, and an absent list is
 * `[]`. Node 24 warns "Passing invalid argument types to fs.existsSync is
 * deprecated" on any non-path argument, so the type gate belongs here rather
 * than at each call site.
 */
function fsExists(fp) {
  return typeof fp === "string" && fp.length > 0 && fs.existsSync(fp);
}

/** Render one `state.files` value without printing blank rows for absent lists. */
function shownPath(fp) {
  if (typeof fp === "string") return fp.length > 0 ? fp : "(none)";
  if (Array.isArray(fp)) return fp.length > 0 ? fp.join(", ") : "(none)";
  return "(none)";
}
...
function renderStatus(state) {
  ...
  if (state.ai_base) {
    ...
    for (const [key, fp] of Object.entries(state.files)) {
      out.push(`    ${fsExists(fp) ? "✓" : "✗"} ${key.padEnd(22)} ${shownPath(fp)}`);
    }
  } else {
