...
// ── web_fetch #38：TUN fake-ip 动态放行 ─────────────────────────────
// Clash/mihomo 等以 fake-ip 接管系统 DNS：解析返回池内地址（198.18.0.0/15、
// fdfe:dcba:9876::/48），再由 TUN 转发到真实公网。上游只接受公网 unicast 地址，
// 于是每个域名都被判成 SSRF 目标而拒绝，表现为 "resolves to a non-public IP address"。
// 这里不写死白名单，而是按需探测：仅当哨兵域名此刻仍解析回池内地址（说明 fake-ip
// 接管仍然生效）才放行池内地址；TUN 关闭后探测失败，立即恢复上游的拒绝行为。
const WEB_FETCH_FAKE_IP_HELPER = [
  "// [dsh-purge] TUN fake-ip 动态放行（按需探测，非白名单）。",
  "const FAKE_IP_POOLS = [\"198.18.0.0/15\", \"fdfe:dcba:9876::/48\"];",
  "const FAKE_IP_SENTINELS = [\"example.com\", \"www.cloudflare.com\", \"one.one.one.one\"];",
  "const fakeIpProbe = { at: 0, active: false };",
  "function isFakeIpPoolAddress(input) {",
  "\tlet parsed;",
  "\ttry {",
  "\t\tparsed = ipaddr.parse(stripIpv6Brackets(String(input)));",
  "\t} catch {",
  "\t\treturn false;",
  "\t}",
  "\tconst addr = parsed instanceof ipaddr.IPv4 ? parsed : parsed.isIPv4MappedAddress() ? parsed.toIPv4Address() : parsed;",
  "\treturn FAKE_IP_POOLS.some((cidr) => {",
  "\t\tconst range = ipaddr.parseCIDR(cidr);",
  "\t\treturn addr.kind() === range[0].kind() && addr.match(range);",
  "\t});",
  "}",
  "async function fakeIpTunnelActive(now = Date.now()) {",
  "\tif (now - fakeIpProbe.at < 60000) return fakeIpProbe.active;",
  "\tlet active = false;",
  "\tfor (const host of FAKE_IP_SENTINELS) {",
  "\t\ttry {",
  "\t\t\tconst answers = await lookup(host, { all: true, order: \"verbatim\" });",
  "\t\t\tif (answers.some((entry) => isFakeIpPoolAddress(entry.address))) {",
  "\t\t\t\tactive = true;",
  "\t\t\t\tbreak;",
  "\t\t\t}",
  "\t\t} catch {",
  "\t\t\t// 单个哨兵失败就试下一个；三个都失败按未激活处理。",
  "\t\t}",
  "\t}",
  "\tfakeIpProbe.at = now;",
  "\tfakeIpProbe.active = active;",
  "\treturn active;",
  "}",
].join("\n") + "\n";
...
const NEW_TOOL_PATCHES = [
  ...
  },
  {
    ...
    replacements: [
      {
        // 行尾换行必须入锚：否则 "timeoutMs: 60000" 会命中 "timeoutMs: 600000" 的前缀，
        // 每次启动 apply 都再追加一个 0（曾把该值推到 6e12）。
        pattern:
          '    - id: bash-sandbox\n' +
  ...
  {
    id: 38, name: "WEB_FETCH_FAKE_IP_TUNNEL", layer: "代码", layer_en: "Code",
    desc: "web_fetch：TUN fake-ip 段动态放行（哨兵探测，非白名单） / dynamic fake-ip pass-through",
    file: "web-fetch-http",
    optional: true,
    replacements: [
      {
        // 锚点必须连上紧随其后的 JSDoc 起始行：只锚常量行的话，注入后该行仍在，
        // 每次 apply 都会再插一份 helper。常量行 + "/**" 在文件里唯一。
        pattern:
          'const IPV4ONLY_SENTINELS = new Set(["192.0.0.170", "192.0.0.171"]);\n' +
          '/**',
        replace:
          'const IPV4ONLY_SENTINELS = new Set(["192.0.0.170", "192.0.0.171"]);\n' +
          WEB_FETCH_FAKE_IP_HELPER +
          '/**',
      },
      {
        // 公网校验：池内地址只在 fake-ip 接管仍生效时放行，其余私网/环回/链路本地照旧拒绝。
        pattern:
        ...
        replace:
          '\t\tif (!isPublicIpAddress(entry.address) && !(isFakeIpPoolAddress(entry.address) && await fakeIpTunnelActive())) throw new WebError(`URL hostname "${hostname}" resolves to a non-public IP address`, "WEB_BLOCKED_URL");',
      },
    ],
    markers: ["isFakeIpPoolAddress(entry.address) && await fakeIpTunnelActive()"],
  },
];
...
export function targetFiles(aiBase, dshHome = findDshHome()) {
  ...
  return {
    ...
    // #38 目标：web_fetch 的公网校验与传输实现（fake-ip 直连路径）。
    "web-fetch-http": firstExisting([
      path.join(aiBase, "dsh-web-fetch-http", "lib", "index.js"),
      ...(root ? [path.join(root, "packages", "web", "web-fetch-http", "lib", "index.js")] : []),
    ]),
